...
} from '\var\task\src\haytham\hdynix-systems/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '\var\task\src\haytham\hdynix-systems/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('\var\task\src\haytham\hdynix-systems')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '\var\task\src\haytham\hdynix-systems',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('\var\task\src\haytham\hdynix-systems\.netlify\dist\run\handlers\server.js')
